...
/**
 * Operations dashboard shell — a developer/ops module separate from the app.
 *
 * Lives under /dashboard and contains only operations tooling (system
 * overview + Redis queues). The app (/app) and the admin dashboard (/admin)
 * are separate modules with their own sidebars.
 */
    ...
    <div className="flex min-h-screen bg-surface-950">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-surface-700 bg-surface-900">
        <Link to="/" className="flex h-16 items-center gap-2.5 border-b border-surface-700 px-5 transition hover:bg-surface-800/50">
          ...
          <span className="rounded-md bg-sky-500/15 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-sky-300">
            Ops
          </span>
        </Link>
        ...
        <div className="border-t border-surface-700 p-3 space-y-3">
          ...
          <p className="px-1 text-[11px] text-zinc-500">LinkEasy Ops • v1.0</p>
        </div>
      </aside>
